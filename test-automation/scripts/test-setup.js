...
const runSetupTests = () => {
  ...
  try {
    ...
    const depsOk = requiredDeps.every(dep => packageJson.dependencies[dep])
    const devDepsOk = requiredDevDeps.every(dep => packageJson.devDependencies[dep])
  ...
  try {
    ...
    const allFunctionsExist = essentialFunctions.every(func => 
  ...
  try {
    ...
    const allTestFilesExist = testFiles.every(file => fs.existsSync(file))
